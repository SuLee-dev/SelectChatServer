...
namespace NLogicLib
{
	...
	void PacketProcess::Init(TcpNet* pNetwork, UserManager* pUserMgr, LobbyManager* pLobbyMgr, ServerConfog* pConfig, ILog* pLogger)
	{
		...
		if (pUser)
		{
			...
			if (pLobby)
			{
				...
				if (pRoom)
				{
					...
					m_pRefLogger->Write(LOG_TYPE::L_INFO, "%s | NtfSysCloseSession. sessionIndex(%d). Room Out", __FUNCTION__, packetInfo.SessionIndex);
				}

				pLobby->LeaveUser(pUser->GetIndex());

				m_pRefLogger->Write(LOG_TYPE::L_INFO, "%s | NtfSysCloseSession. sessionIndex(%d). Lobby Out", __FUNCTION__, packetInfo.SessionIndex);
			}
